...
describe('cachedFetch (memory cache path)', () => {
  ...
  it('calls the fetch function only once for the same key', async () => {
    ...
    const a = await cachedFetch('test-key', fetchFn, 60);
    const b = await cachedFetch('test-key', fetchFn, 60);
  ...
  it('can cache different keys independently', async () => {
    ...
    const v1 = await cachedFetch('key-1', fetchFn, 60);
    const v2 = await cachedFetch('key-2', fetchFn, 60);
